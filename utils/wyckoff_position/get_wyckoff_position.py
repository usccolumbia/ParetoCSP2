...
class GetWyckoffPosition:
    ...
    def get_wyckoffs(self, atom_num, is_random):
        ...
        if is_random:
            self.wyckoffs = self.combination_wp_random(
                wyckoff_position, atom_num, is_shuffle=True
            )
        else:
            self.wyckoffs = self.combination_wp_all(wyckoff_position, atom_num)

    def combination_wp_all(
        self,
        ...
        wp_part_list = []
        for an in atom_num:
            part_wp_an_path = os.path.join(
                self.save_path, "part_wp", str(an) + "_" + str(self.sg)
            )
    ...
    def combination_wp_random(
        ...
        for an in atom_num:
            ...
            wp_part = self.combination_wp_part(
                wyckoff_position, an, max_count=max_count
            )
            # wp_part = wp_part[0]
    ...
    @staticmethod
    def combination_wp_part(
        wyckoff_position: list, atom_num_part: int, max_count: int = -1
    ) -> list:

        def dfs(target: int, index: int, temp: list, temp_num: list):
            if sum(temp_num) == de:
                temp.sort()
                if temp not in result:
                    result.append(temp)
                if (
                    len(result) == max_count
                ):  # 达到max_count个组合数，引发异常，结束搜索
                    raise Exception()

            for i in range(index, len(wp_num)):
                if wp[i] in temp and not (
                    "x" in ",".join(wp[i])
                    or "y" in ",".join(wp[i])
                    or "z" in ",".join(wp[i])
                ):
...
def get_all_wyckoff_combination(sg_list, atom_num):
    ...
    wyckoff_combination_type = "".join(
        [alphabet[i] + str(atom_num[i]) for i in range(len(atom_num))]
    )

    wyckoffs_dict = {}
    max_wyckoffs_count = 0
    for sg_i in sg_list:
        sg_i_path = os.path.join(
            current_path, "wp_sg", wyckoff_combination_type + "_" + str(sg_i)
        )
        if os.path.isfile(sg_i_path):
            wp = read_data_bin(None, sg_i_path)
        else:
            wp = GetWyckoffPosition(
                sg_i, atom_num, is_random=False, verbose=False, save_path=current_path
            ).wyckoffs
